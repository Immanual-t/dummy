...
function generateInitialTips(customerType, productType) {
    ...
    .then(data => {
        ...
        // Display AI response - use formatted_response if available
        displayAIMessage(data.response, data.formatted_response);
...
function displayAIMessage(message, formattedMessage = null) {
    const messageElement = document.createElement('div');
    messageElement.className = 'd-flex flex-column mb-3';

    // If we have a formatted message, use it, otherwise use the plain message
    const displayContent = formattedMessage ?
        `<div class="ai-response-content">${formattedMessage}</div>` :
        message;
