...
class AIService:
    ...
    def generate_response(messages, max_tokens=500):
        ...
        try:
            ...
            return deepseek_client.generate_completion(
                messages=messages,
    ...
    def format_response_for_html(response_text):
        ...
        """
        return ResponseFormatter.format_for_html(response_text)
...
class SalesCopilotService(AIService):
    ...
    def generate_sales_suggestion(cls, user, messages):
        ...
        try:
            # Ensure messages is a list and has the expected structure
            if not isinstance(messages, list):
                return {'status': 'error', 'message': 'Invalid messages format. Expected a list.'}

            # Simple validation to ensure each message has role and content
            for message in messages:
                if not isinstance(message, dict) or 'role' not in message or 'content' not in message:
                    return {'status': 'error',
                            'message': 'Invalid message format. Each message should have role and content.'}

            # Generate response using the AI model
            response = cls.generate_response(messages, max_tokens=600)

            # Log the interaction in the database
            cls.log_interaction(user, 'copilot', messages, response)
    ...
    def generate_quiz_questions(cls, user, product_type, skill_level):
        ...
        try:
            prompt = [
                ...
                {"role": "user",
                 "content": f"Create 5 multiple-choice questions to assess knowledge about {product_type} products for a {skill_level} level agent. Each question should have 4 options with 1 correct answer. Format the response as a JSON array of question objects with these fields: 'question', 'options' (array), and 'correct_answer' (integer index)."}
            ]
            response = cls.generate_response(prompt, max_tokens=800)

            try:
                # Try to parse the JSON response
                # First, attempt to find a JSON pattern in the response
                import re
                ...
                if json_match:
                    try:
                        ...
                        # Try a more forgiving approach by cleaning the string first
                        cleaned_json = re.sub(r'([{,])\s*([a-zA-Z0-9_]+)\s*:', r'\1"\2":', json_match.group(1))
                        cleaned_json = re.sub(r':\s*\'([^\']*)\'\s*([,}])', r': "\1"\2', cleaned_json)
                        cleaned_json = re.sub(r':\s*"([^"]*)"\'s\s*([,}])', r': "\1\'s"\2', cleaned_json)
                        try:
                            questions = json.loads(cleaned_json)
                        except json.JSONDecodeError:
                            questions = []
                else:
                    # If no JSON-like pattern is found, try to create structured questions from scratch
                    questions = [
                        {
                            "question": f"Sample question about {product_type}?",
                            "options": ["Option A", "Option B", "Option C", "Option D"],
                            "correct_answer": 0
                        },
                        {
                            "question": f"Another sample question about {product_type}?",
                            "options": ["Option A", "Option B", "Option C", "Option D"],
                            "correct_answer": 0
                        }
                    ]
                ...
                logger.error(f"Error parsing quiz questions: {str(parse_error)}")
                questions = []
