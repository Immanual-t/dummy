...
def copilot_view(request):
    ...
    if conversation_id:
        try:
            ...
            customer_type_display = dict(customer_types)[customer_type]
            product_type_display = dict(product_types)[product_type]
            conversation_active = True
        except Conversation.DoesNotExist:
            # Clear the session if conversation not found
    ...
    context = {
        ...
        'css_files': ['css/ai_response.css']  # Add our new CSS
    }

    # Add conversation data if active
    ...
    if request.method == 'POST':
        try:
            ...
            if result['status'] == 'success':
                # Format response using our new formatter
                formatted_response = ResponseFormatter.format_for_html(result['response'])
                ...
                return JsonResponse({
                    ...
                    'metadata': {
                        'has_markdown': '**' in result['response'] or '*' in result['response'] or '#' in result['response'],
                        'has_lists': ('* ' in result['response'] or '- ' in result['response'] or '• ' in result['response']),
                        'word_count': len(result['response'].split())
                    }
                })
            else:
                return JsonResponse({'status': 'error', 'message': 'Failed to generate suggestion.'})
...
def save_conversation_view(request):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.success(request, f'Conversation "{title}" has been saved.')
            return redirect('ai_assistant:conversation_detail', conversation_id=conversation.id)

        except Exception as e:
            messages.error(request, f'Error saving conversation: {str(e)}')
            return redirect('ai_assistant:copilot')
...
def conversation_detail_view(request, conversation_id):
    ...
    context = {
        ...
        'page_title': f"Conversation: {conversation.title}",
        'css_files': ['css/ai_response.css']  # Add our new CSS
    }
...
@login_required
def continue_conversation_view(request, conversation_id):
    """View for continuing an existing conversation"""
    conversation = get_object_or_404(Conversation, id=conversation_id, user=request.user)

    # Store conversation ID in session
    request.session['active_conversation_id'] = conversation.id

    # Redirect to copilot view
    return redirect('ai_assistant:copilot')
...
def delete_conversation_view(request, conversation_id):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f'Conversation "{conversation_title}" has been deleted.')
...
def generate_learning_content_view(request):
    ...
    if request.method == 'POST':
        ...
        if result['status'] == 'success':
            ...
            # Generate a summary using the first paragraph or first 150 chars
            first_para_end = content.find('\n\n')
            if first_para_end > 0 and first_para_end < 300:
                summary = content[:first_para_end].strip()
            else:
...
def learning_content_detail_view(request, content_id):
    ...
    formatted_content = ResponseFormatter.format_for_html(content.content)

    context = {
        'content': content,
        'formatted_content': formatted_content,  # Pass the pre-formatted content
        'related_content': related_content,
        'page_title': content.topic,
        'css_files': ['css/ai_response.css']  # Add our new CSS
    }
...
def generate_lead_message_view(request, lead_id):
    ...
    if result['status'] == 'success':
        # Format the response using our new formatter
        formatted_message = ResponseFormatter.format_for_chat(result['response'])
...
def analyze_leads_view(request):
    ...
    if result['status'] == 'success':
        ...
        formatted_analysis = ResponseFormatter.format_for_html(result['response'])

        context = {
            'analysis': result['response'],
            'formatted_analysis': formatted_analysis,  # Pass the pre-formatted content
            'leads': leads,
